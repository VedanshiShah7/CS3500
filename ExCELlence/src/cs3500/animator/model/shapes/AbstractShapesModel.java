...
public abstract class AbstractShapesModel implements ShapesModel {
  ...
  private void movePerTick(int x, int y, double start, double stop, double clock) {
    ...
    double xAtT =
            startX * ((stop - clock) / (stop - start)) + x * ((clock - start) / (stop - start));
    double yAtT =
            startY * ((stop - clock) / (stop - start)) + y * ((clock - start) / (stop - start));
    this.position = new CartPosition(xAtT,
            yAtT);
  }
  ...
  private void changeSizePerTick(int w, int h, double start, double stop, double clock) {
    ...
    double widthAtT =
            startWidth * ((stop - clock) / (stop - start)) + w * ((clock - start)
                    / (stop - start));
    double heightAtT =
            startHeight * ((stop - clock) / (stop - start)) + h * ((clock - start)
                    / (stop - start));
  ...
  private void changeColorPerTick(int r, int g, int b, double start, double stop,
    ...
    double rAtT =
            startR * ((stop - clock) / (stop - start)) + r * ((clock - start) / (stop - start));
    double gAtT =
            startG * ((stop - clock) / (stop - start)) + g * ((clock - start) / (stop - start));
    double bAtT =
            startB * ((stop - clock) / (stop - start)) + b * ((clock - start) / (stop - start));
  ...
  protected HashMap<Double, Double[]> getFrame() {
